...
from .common import (
    BasicFunctionality, BaseExchangeTypes, BasePriority, BaseTimeToLive
)


def get_connection(
        hostname, port, vhost):
    return kombu.Connection(f'tarantool://{hostname}:{port}')
...
def connection(request):
    ...
    return get_connection(
        hostname=os.environ.get('TARANTOOL_HOST', 'localhost'),
...
def invalid_connection():
    return kombu.Connection('tarantool://localhost:12345')


@pytest.mark.env('tarantool')
@pytest.mark.flaky(reruns=5, reruns_delay=2)
class test_TarantoolBasicFunctionality(BasicFunctionality):
    pass


@pytest.mark.env('tarantool')
@pytest.mark.flaky(reruns=5, reruns_delay=2)
class test_TarantoolBaseExchangeTypes(BaseExchangeTypes):
    pass


@pytest.mark.env('tarantool')
@pytest.mark.flaky(reruns=5, reruns_delay=2)
class test_TarantoolBaseTimeToLive(BaseTimeToLive):
    pass


@pytest.mark.env('tarantool')
@pytest.mark.flaky(reruns=5, reruns_delay=2)
class test_TarantoolPriority(BasePriority):
    # Comparing to py-amqp transport has Tarantool transport several
    # differences:
    # 1. Order of priorities is reversed
    # 2. drain_events() consumes only single value

    # Tarantool transport has lower numbers higher priority
    PRIORITY_ORDER = 'desc'
